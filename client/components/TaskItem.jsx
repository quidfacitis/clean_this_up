...
const TaskItem = ({task, deleteTask, toggleTaskAssignmentForm}) => {
  return (
    <div className="staff-member-card">
      <div className="staff-name">{task.title}</div>
      <div className="staff-role">{task.description}</div>
      <div className="staff-card-assign-task" style={{backgroundColor: '#445bb2', width: '8em'}} onClick={() => toggleTaskAssignmentForm(task)}>Assign to Staff</div>
